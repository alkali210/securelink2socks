...
**当前是兼容性验证阶段，尚不是可用的 SOCKS 代理。** 按初始计划，真实 XMU 握手、原始 ACL 和用户态 TCP 验证完成前，不进入 SOCKS/重连服务实现。真实 SSO、API 刷新及配置获取已成功；实验握手完成 TLS 校验和 KEY_METHOD 2 交换，但网关返回无原因的 `AUTH_FAILED`，尚未获得 cipher、IPv4 或 ACL。
...
# 显式测试 XMU 普通 TLS 外层控制报文和仅 AEAD 协商：
...
`check --aead-probe` 是显式兼容性实验：按 XMU profile 的内嵌 CA 和 `remote-cert-tls server` 验证服务器，要求明确的 serverAuth EKU；允许没有 tls-auth/tls-crypt 的外层报文，内部仍使用已验证的 TLS。它只声明已实现的 AEAD 算法，不支持或回退到 CBC，不修改原始 profile。普通 `check` 仍保留严格 parser 策略。
...
目前尚不能证明 XMU 接受仅 AEAD 客户端；不会把实验结果当成验收成功或猜测 ACL 语义。详情见 [协议调查](docs/protocol-notes.md)、[实机调查](docs/live-validation.md) 和 [依赖补丁](docs/upstream-patch.md)。
